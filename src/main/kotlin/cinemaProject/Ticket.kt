...
data class Ticket(
    ...
    init {
        ...
        if (saloon.chairCount < 1) {
            ...
            if (isChairAvailable(chairNo)) {//kesinlikle bilet alinacak
                ...
                println("$messagePrefix ${owner.name} ${owner.surname} biletiniz hazir iyi seyirler bilet ucretiniz: ${event.price}. $discountText")
    ...
    private fun getDiscountText(type: TicketType, totalPrice: Double, price: Double) {
        when (type) {
